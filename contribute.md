...
Both are built for **linux/amd64**, **linux/arm64**, **linux/arm/v7**, and **linux/386**.

**GHCR image** (default `ghcr.io/<owner>/<repo>`, same tags on one package):

| Git tag | Workflow | GitHub | Server-only tags | Server + Studio tags |
|---------|----------|--------|------------------|----------------------|
...
**Floating tags:** `:latest` always tracks the **unified** (server + Studio) image from the latest production git tag. `:latest-slim` tracks the **server-only** image from the same release. RC builds do **not** update `:latest` or `:latest-slim`.

Release workflows log in to **`ghcr.io`** with the default **`GITHUB_TOKEN`** and need **`permissions: packages: write`** (already set in the workflow). No AWS credentials. Optional repo variable **`GHCR_IMAGE_NAME`** sets the image path under `ghcr.io` when it must differ from `github.repository` (must be lowercase `owner/name`). Pull requests **verify both** Dockerfiles ([docker-verify.yml](.github/workflows/docker-verify.yml), host platform only).
...
If a release build fails on a specific architecture (for example missing DuckDB or PyO3 wheels for **linux/386** or **linux/arm/v7**), reduce `IMAGE_PLATFORMS` in [docker-release-rc.yml](.github/workflows/docker-release-rc.yml) and [docker-release-prod.yml](.github/workflows/docker-release-prod.yml).
