...
CI/build: each release pushes **two** multi-arch manifests to **GHCR** (`ghcr.io/<owner>/<repo>`): **server-only** (`:vX.Y.Z`, prod also `:latest-slim`) and **server + Studio** (`:vX.Y.Z-full`, prod also `:latest`), plus per-arch tags (`-amd64` / `-arm64`) for pinning. Native **amd64** / **arm64** runners + `buildx imagetools` (see [contribute.md](../contribute.md)). Workflows: [`.github/workflows/docker-release-rc.yml`](../.github/workflows/docker-release-rc.yml), [`.github/workflows/docker-release-prod.yml`](../.github/workflows/docker-release-prod.yml). PRs: [`.github/workflows/docker-verify.yml`](../.github/workflows/docker-verify.yml).
