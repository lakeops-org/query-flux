    ...
    pub fn into_response(self) -> Response<Body> {
        let resp = if let Some(msg) = self.error {
            ...
            TrinoResponse {
                ...
                error: None,
                columns: if self.columns.is_empty() { None } else { Some(Value::Array(self.columns)) },
                data: if self.rows.is_empty() { None } else { Some(Value::Array(self.rows)) },
                update_type: None,
                update_count: None,
                warnings: vec![],
            }
...
fn arrow_cell_to_json(
    ...
    match col.data_type() {
        DataType::Boolean => {
            let v = col.as_any().downcast_ref::<BooleanArray>().map(|a| a.value(row));
            v.map(Value::Bool).unwrap_or_else(|| Value::String(formatter.value(row).to_string()))
        }
        DataType::Int8 => {
            json!(col.as_any().downcast_ref::<Int8Array>().map(|a| a.value(row) as i64))
        }
        DataType::Int16 => {
            json!(col.as_any().downcast_ref::<Int16Array>().map(|a| a.value(row) as i64))
        }
        DataType::Int32 => {
            json!(col.as_any().downcast_ref::<Int32Array>().map(|a| a.value(row) as i64))
        }
        DataType::Int64 => {
            json!(col.as_any().downcast_ref::<Int64Array>().map(|a| a.value(row)))
        }
        DataType::UInt8 => {
            json!(col.as_any().downcast_ref::<UInt8Array>().map(|a| a.value(row) as i64))
        }
        DataType::UInt16 => {
            json!(col.as_any().downcast_ref::<UInt16Array>().map(|a| a.value(row) as i64))
        }
        DataType::UInt32 => {
            json!(col.as_any().downcast_ref::<UInt32Array>().map(|a| a.value(row) as i64))
        }
        DataType::UInt64 => {
        ...
        DataType::Float32 => {
            ...
            json!(col.as_any().downcast_ref::<Float32Array>().map(|a| a.value(row) as f64))
        }
        DataType::Float64 => {
            json!(col.as_any().downcast_ref::<Float64Array>().map(|a| a.value(row)))
        }
